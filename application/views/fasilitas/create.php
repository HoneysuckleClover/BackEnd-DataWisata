<div class="content-wrapper" style="background:#e9f3f2; min-height:100vh; padding:35px 25px;">
  ...
  <div class="card border-0 shadow-sm" style="border-radius:14px; overflow:hidden;">
    <div class="card-header text-white" 
      ...
      <i class="fas fa-tree me-1"></i> Form Tambah Data Fasilitas
    </div>

    <div class="card-body" style="padding:30px;">
      <form action="<?= site_url('fasilitas/save') ?>" method="post">
        <div class="row justify-content-center">
          <div class="col-md-8">
            <!-- Nama Fasilitas -->
            <div class="mb-4">
              <label for="nama_fasilitas" class="form-label fw-bold" style="color:#0e3c38; margin-bottom:8px;">
                <i class="fas fa-check-circle me-1 text-muted"></i>Nama Fasilitas <span class="text-danger">*</span>
              </label>
              <input type="text" 
                     class="form-control" 
                     id="nama_fasilitas" 
                     name="nama_fasilitas" 
                     required
                     placeholder="Contoh: Parkir Luas, Toilet Bersih, Mushola"
              ...
              <div class="form-text text-muted mt-2" style="font-size:13px;">
                <i class="fas fa-lightbulb me-1"></i>Masukkan nama fasilitas yang jelas dan mudah dipahami
              </div>
            </div>
            ...
            <div class="alert alert-info border-0 mt-4" style="background:#e3f2fd; border-radius:10px; border-left:4px solid #2196f3;">
              <div class="d-flex align-items-center">
                ...
                <div>
                  ...
                  <small class="text-muted d-block">Pastikan nama fasilitas belum terdaftar sebelumnya.</small>
                  <small class="text-muted d-block">Fasilitas akan digunakan untuk data wisata.</small>
                </div>
              </div>
            </div>
            ...
            <div class="alert alert-light border-0 mt-3" style="background:#f8f9fa; border-radius:10px; border-left:4px solid #6c757d;">
              <div class="d-flex align-items-center">
                ...
                <div>
                  <small class="fw-bold text-dark">Contoh Fasilitas Populer:</small>
                  <div class="row mt-2">
                    <div class="col-md-6">
                      <small class="text-muted d-block">• Parkir Luas</small>
                      <small class="text-muted d-block">• Toilet Bersih</small>
                      <small class="text-muted d-block">• Mushola</small>
                      <small class="text-muted d-block">• Warung Makan</small>
                    </div>
                    <div class="col-md-6">
                      <small class="text-muted d-block">• Area Bermain Anak</small>
                      <small class="text-muted d-block">• Gazebo</small>
                      <small class="text-muted d-block">• Spot Foto</small>
                      <small class="text-muted d-block">• WiFi Gratis</small>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <!-- Tips -->
            <div class="alert alert-success border-0 mt-3" style="background:#d4edda; border-radius:10px; border-left:4px solid #28a745;">
              <div class="d-flex align-items-center">
                <i class="fas fa-tips me-2 text-success fs-5"></i>
                <div>
                  <small class="fw-bold text-dark">Tips:</small>
                  <small class="text-muted d-block">• Gunakan nama yang spesifik dan deskriptif</small>
                  <small class="text-muted d-block">• Hindari nama yang terlalu umum</small>
                  <small class="text-muted d-block">• Pisahkan fasilitas yang berbeda</small>
                </div>
              </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const namaFasilitasInput = document.getElementById('nama_fasilitas');
  
  form.addEventListener('submit', function(e) {
    const namaFasilitas = namaFasilitasInput.value.trim();
    
    // Validasi nama fasilitas tidak boleh kosong
    if (!namaFasilitas) {
      e.preventDefault();
      showError('Nama fasilitas harus diisi!');
      namaFasilitasInput.focus();
      return false;
    }
    
    // Validasi panjang nama fasilitas
    if (namaFasilitas.length < 3) {
      e.preventDefault();
      showError('Nama fasilitas minimal 3 karakter!');
      namaFasilitasInput.focus();
      return false;
    }
    
    // Validasi format nama fasilitas (hanya huruf, spasi, dan tanda hubung)
    const namaRegex = /^[a-zA-Z\s\-']+$/;
    if (!namaRegex.test(namaFasilitas)) {
      e.preventDefault();
      showError('Nama fasilitas hanya boleh mengandung huruf, spasi, dan tanda hubung!');
      namaFasilitasInput.focus();
      return false;
    }
    
    // Validasi panjang maksimal
    if (namaFasilitas.length > 50) {
      e.preventDefault();
      showError('Nama fasilitas maksimal 50 karakter!');
      namaFasilitasInput.focus();
      return false;
    }
  });
  ...
  namaFasilitasInput.addEventListener('input', function() {
